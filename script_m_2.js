...
$("body").bind('touchmove mousemove', function(e){
	...
	var h = window.innerHeight
		for (var i = jQuery('.insta_post_2>img').length; i >= 1; i--) {
			jQuery('.insta_post_2>img:nth-child('+i+')').css({'opacity':Math.abs(((e.originalEvent.touches[e.originalEvent.touches.length-1].pageY + 100) - (w/jQuery('.insta_post_2>img').length * i))/(w*1.2))})
		}
    })
})